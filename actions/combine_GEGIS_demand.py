...
if __name__ == "__main__":
    ...
    region = sorted(list(snakemake.config["regions"][snakemake.wildcards["region"]].keys()))
    year = int(snakemake.wildcards["era_year"])

    with h5py.File(snakemake.input[0], "r") as f:
        da = xr.DataArray(np.array(f['demand']),
                            name="demand",
                            dims=["region", "time"],
                            coords={"region": region,
                            "time":   pd.date_range(start=str(year),end=str(year+1), freq="H", closed="left")
                            }
                            )
